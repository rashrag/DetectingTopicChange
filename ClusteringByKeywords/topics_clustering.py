...
def createTopicsFile(annotation_file, text, win_size, print=False):
    ...
    topics_list = []
    sentence_index = 0
    for sentence in mergeSentences(text, win_size):
        if print:
            print(sentence)
...
if __name__=="__main__":
    ...
    win_size = 4
    createTopicsFile(annotation_filename, text, win_size)
    ...
    #print(frame)
    plt.scatter(x=frame.index, y=frame['cluster'], c=frame['cluster'])
    plt.xlabel('Time')
    plt.ylabel('Topic Cluster #')
    plt.title('15-means Clustering (Window size ' + str(win_size) + ') - Topics Distribution')
    plt.show()
